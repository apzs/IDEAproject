...
public class RoleDaoImpl implements com.dao.RoleDao {
    ...
    public void save(Role role) {
        jdbcTemplate.update("insert into test.sys_role(roleName,roleDesc) values(?,?)",role.getRoleName(),role.getRoleDesc());
    }

    @Override
    public List<Role> findRoleById(Long id) {
        List<Role> roles = jdbcTemplate.query(" select roleName " +
                                                  " from (select roleId from test.sys_user_role where userId=?) as rid " +
                                                  " join test.sys_role as sr " +
                                                  "on rid.roleId = sr.id"
